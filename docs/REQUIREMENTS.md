...
- **FR-17 (GitHub Repository & Project Linkage & Database Persistence)**: The system shall allow users to link a SCJ28 Project to a GitHub repository (`owner/repository`) and optional GitHub Project (v2) Board ID via either the dedicated "Connect GitHub Repository" modal or the Project Creation/Edit modal. Connection details (`github_repo`, `github_project_id`) are permanently persisted in the SQLite `projects` database table.
